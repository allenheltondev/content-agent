...
export const converse = async (model, systemPrompt, userPrompt, tools, options) => {
  let conversation = [];
...
export const getMemoryId = async () => {
  if (MEMORY_ID) return MEMORY_ID;

  const parameter = await getParameter(process.env.MEMORY_PARAMETER);
  MEMORY_ID = parameter;
  return MEMORY_ID;
};
